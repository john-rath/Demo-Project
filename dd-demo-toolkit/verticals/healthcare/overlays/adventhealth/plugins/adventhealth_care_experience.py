"""
AdventHealth "Sensing Hospital" Care-Experience Cascade — sub-vertical: adventhealth
...
the same lesson the BD Pyxis plugin documents: restricting to a single
floor/wing would mutate only ~1 of N devices and let the cascade signal get
drowned by gauss-random-walk drift on the unaffected majority. Even where a
MedSurg room is physically on Floor 3 East, there is no collision: this
plugin only touches care_sensing device types and the hospital.sensing.* /
hospital.cxp.* namespaces, while wifi_cascade only touches pumps/APs and
hospital.network.* — so device_type + namespace + incident_domain fully
bifurcate the two stories.
...
Cascade narrative (see notebook adventhealth-care-experience-rca):
...
logger = logging.getLogger("adventhealth_care_experience_incident")
...
class AdventHealthCareExperienceCascade(IncidentPlugin):
    """
    AdventHealth Sensing-Hospital care-experience cascade.
    ...
    def get_incident_name(self) -> str:
        return (
            "AdventHealth Sensing-Hospital RTLS Location-Sync Degradation → "
    ...
    def on_tick(
        ...
        if hasattr(engine, "incident_state"):
            if phase == "normal":
                engine.incident_state.pop("adventhealth_care_experience", None)
            else:
                engine.incident_state["adventhealth_care_experience"] = {
                    "phase": phase,
    ...
    def _matches_type(self, device: Any, dtype_wanted: str) -> bool:
        ...
        Filters by department only (not floor/wing) so the cascade reads as a
        clear MedSurg-wide anomaly rather than mutating a single room."""
